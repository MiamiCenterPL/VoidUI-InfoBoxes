...
if VoidUI_IB.options.lootbags_infobox or VoidUI_IB.options.collectables or VoidUI_IB.options.SeparateBagged then
    ...
    --Sanity check, on some modded instances might throw an exception;
    if not alive(unit) or not unit.carry_data or not unit:carry_data() then return end --Check whether function exists
    --
					local carry_id = unit:carry_data():carry_id()
					if carry_id and tweak_data and tweak_data.carry and tweak_data.carry[carry_id].bag_value then
